...
<style>
  ...
  *:focus-visible {
  ...
  @media print {
    ...
    .notification-bell, .session-warning, .quiz-timer, .install-pwa-btn {
...
<body>
  ...
  <div class="login-wrapper">
    <img src="https://via.placeholder.com/150" alt="DSHS Logo" onerror="this.src='https://via.placeholder.com/150'">
  </div>
...
<script>
  ...
  function loadSection(tab){
    ...
    if(tab === "Transcript" && role === "student"){
      ...
      if(student.gradeLevel !== "12"){
        ...
        transcriptDiv.innerHTML = `
          ...
          <div style="margin: 20px 0;">
            ...
            <p><strong>Track/Strand:</strong> ${student.track} - ${student.strand}</p>
            <p><strong>Date of Graduation:</strong> June 2024</p>
          </div>
          <h4>Academic Record</h4>
        `;
        
        // Add grades from all semesters
        const gradesTable = document.createElement('table');
        gradesTable.innerHTML = '<tr><th>Subject</th><th>Grade 11</th><th>Grade 12</th><th>Final</th></tr>';
        
        subjects.forEach(sub => {
          const g11Grade = Math.floor(Math.random() * 15) + 85; // Simulated Grade 11
          const g12Grade = gradesData[student.name]?.[sub] || 0;
        ...
        transcriptDiv.appendChild(gradesTable);
        ...
        transcriptDiv.innerHTML += `
          <div style="margin-top: 40px; text-align: center;">
            ...
            <p style="margin-top: 30px; font-size: 12px; color: #666;">
              ...
              Reference No: TR-${student.id}-${Date.now()}
            </p>
          </div>
        `;
        ...
        downloadBtn.onclick = () => downloadTranscriptPDF(student);
    ...
    if(tab === "Course Materials" && role === "student"){
    ...
    if(tab === "Behavior Reports"){
      ...
      if(role === "teacher"){
        // Teacher adds behavior records
        const teacher = teachers.find(t => t.name === currentUser);
        const sectionSelect = document.createElement('select');
        sectionSelect.innerHTML = `<option value="${teacher.sectionHandled}">${teacher.sectionHandled}</option>`;
        section.appendChild(sectionSelect);
      ...
      if(role === "student"){
        ...
        section.innerHTML += `
          <div class="section" style="margin-top: 30px; text-align: center;">
            ...
            <p><strong>General Average:</strong> ${rankings.find(s => s.name === currentUser)?.average.toFixed(2)}%</p>
          </div>
        `;
      }
      ...
      if(role === "admin"){
        ...
        section.innerHTML += `
          <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            ...
            <button onclick="showAddScholarship()" style="background: #28a745; color: white; width: auto;">➕ Add Scholarship</button>
          </div>
        `;
        
        const activeScholarships = scholarships.filter(s => s.active);
        if(activeScholarships.length > 0){
          section.innerHTML += "<h4>Active Scholarships</h4>";
          activeScholarships.forEach(s => {
            section.innerHTML += `
              <div class="scholarship-card">
                <h4>${s.name}</h4>
                <p>${s.description}</p>
                <p><strong>Discount:</strong> ${s.discount}% | <strong>Recipients:</strong> ${s.recipients?.length || 0}</p>
              </div>
            `;
          });
        }
        ...
        if(role === "parent"){
          const parent = parents.find(p => p.child === currentUser);
          targetUser = parent.child;
        }
        const student = students.find(s => s.name === targetUser);
        
        // Check for scholarships
        const myScholarships = scholarships.filter(s => s.recipients?.includes(targetUser));
        if(myScholarships.length > 0){
          section.innerHTML += `
            <div class="scholarship-card">
              <h4>🎓 Your Scholarships</h4>
              ${myScholarships.map(s => `
                <p><strong>${s.name}</strong> - ${s.discount}% discount</p>
              `).join('')}
            </div>
          `;
        }
        ...
        const balance = 15000 - myPayments.reduce((a,p) => a + p.amount, 0); // Simulated tuition
        
        section.innerHTML += `
          <div class="performance-box" style="background: ${balance > 0 ? 'linear-gradient(135deg, #dc3545 0%, #fd7e14 100%)' : 'linear-gradient(135deg, #28a745 0%, #20c997 100%)'}">
        ...
        if(myPayments.length > 0){
          section.innerHTML += "<h4>Payment History</h4>";
          myPayments.sort((a,b) => new Date(b.date) - new Date(a.date)).forEach(p => {
            section.innerHTML += `
      ...
      if(role === "admin"){
        ...
        section.innerHTML += `
          <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            ...
            <button onclick="addBook()" style="background: #28a745; color: white; margin-top: 10px;">➕ Add Book</button>
          </div>
        `;
      }
    ...
    //------ ROOM BOOKING ------
    if(tab === "Room Booking" && role === "admin"){
      section.innerHTML = "<h3>🏫 Room & Facility Booking</h3>";
      
      const rooms = [
        {id: 'science-lab', name: 'Science Laboratory', capacity: 40, type: 'Laboratory'},
        {id: 'computer-lab', name: 'Computer Laboratory', capacity: 30, type: 'Laboratory'},
        {id: 'library', name: 'Library', capacity: 50, type: 'Study'},
        {id: 'gym', name: 'Gymnasium', capacity: 200, type: 'Sports'},
        {id: 'auditorium', name: 'School Auditorium', capacity: 500, type: 'Events'}
      ];
      
      rooms.forEach(room => {
        const bookings = roomBookings.filter(b => b.roomId === room.id && new Date(b.date) >= new Date());
        section.innerHTML += `
          <div class="room-card">
            <div class="room-image">🏫</div>
            <h4>${room.name}</h4>
            <p><strong>Capacity:</strong> ${room.capacity} | <strong>Type:</strong> ${room.type}</p>
            <p><strong>Upcoming Bookings:</strong> ${bookings.length}</p>
            <button onclick="viewRoomSchedule('${room.id}')" style="width: auto; background: var(--main-blue); color: white;">📅 View Schedule</button>
          </div>
        `;
      });
    ...
    if(tab === "Enrollment" && role === "admin"){
      ...
      section.innerHTML += `
        ...
        <div class="enrollment-step" id="enrollStep4">
          ...
          <div id="enrollReview" style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin: 15px 0;">
            <!-- Populated by JS -->
          </div>
          <div style="display: flex; gap: 10px;">
    ...
    if(tab === "Analytics" && role === "admin"){
      ...
      // Grade distribution
      const gradeRanges = {'90-100': 0, '85-89': 0, '80-84': 0, '75-79': 0, 'Below 75': 0};
      students.forEach(s => {
        const grades = Object.values(gradesData[s.name] || {});
        const avg = grades.reduce((a,b) => a+b, 0) / grades.length;
        if(avg >= 90) gradeRanges['90-100']++;
        else if(avg >= 85) gradeRanges['85-89']++;
        else if(avg >= 80) gradeRanges['80-84']++;
        else if(avg >= 75) gradeRanges['75-79']++;
        else gradeRanges['Below 75']++;
      });
      
      section.innerHTML += `
        <div class="chart-container">
          <h4>Grade Distribution</h4>
          <div style="display: flex; align-items: flex-end; height: 200px; gap: 20px; margin-top: 20px; padding: 0 20px;">
            ${Object.entries(gradeRanges).map(([range, count]) => `
              <div style="flex: 1; text-align: center;">
                <div style="background: linear-gradient(to top, var(--main-blue), var(--main-red)); height: ${count > 0 ? (count/students.length*180) : 5}px; border-radius: 5px 5px 0 0; min-height: 5px;"></div>
                <div style="margin-top: 10px; font-size: 12px; font-weight: bold;">${range}</div>
                <div style="font-size: 11px; color: #666;">${count} students</div>
              </div>
            `).join('')}
          </div>
        </div>
    ...
    //------ ALUMNI ------
    if(tab === "Alumni" && role === "admin"){
      section.innerHTML = "<h3>🎓 Alumni Network</h3>";
      ...
      section.innerHTML += `
        <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
          ...
          <select id="alumniStudent">
            ...
            ${students.filter(s => s.gradeLevel === '12').map(s => `<option value="${s.name}">${s.name} (${s.strand})</option>`).join('')}
          </select>
          <input type="text" id="alumniContact" placeholder="Contact Number">
          <input type="email" id="alumniEmail" placeholder="Email Address">
          <input type="text" id="alumniCollege" placeholder="College/University (if applicable)">
          <select id="alumniStatus">
            <option value="">Current Status</option>
            <option value="studying">Currently Studying</option>
            <option value="working">Working</option>
            <option value="looking">Looking for Opportunities</option>
          </select>
          <button onclick="registerAlumni()" style="background: #28a745; color: white; margin-top: 10px;">➕ Register Alumni</button>
        </div>
      `;
      
      // List alumni
      if(alumni.length > 0){
        section.innerHTML += "<h4>Registered Alumni</h4>";
        section.innerHTML += `
          <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 20px;">
            ${alumni.map(a => {
              const student = students.find(s => s.name === a.name);
              return `
                <div class="alumni-card">
                  <div class="alumni-avatar">🎓</div>
                  <h4>${a.name}</h4>
                  <p><strong>Batch:</strong> 2024</p>
                  <p><strong>Strand:</strong> ${student?.strand || 'N/A'}</p>
                  <p><strong>Status:</strong> ${a.status === 'studying' ? '📚 Studying' : a.status === 'working' ? '💼 Working' : '🔍 Job Hunting'}</p>
                  ${a.college ? `<p><strong>School:</strong> ${a.college}</p>` : ''}
                  <p style="font-size: 12px; color: #666; margin-top: 10px;">${a.email}</p>
                </div>
              `;
            }).join('')}
          </div>
      ...
      if(role === "teacher" || role === "admin") {
        ...
        addBtn.onclick = function() {
          if(newMsg.value.trim()) {
            ...
            // Notify all users
            students.forEach(s => addNotification('New announcement: ' + newMsg.value.substring(0, 50) + '...', 'announcement', s.name));
            teachers.forEach(t => addNotification('New announcement: ' + newMsg.value.substring(0, 50) + '...', 'announcement', t.name));
            parents.forEach(p => addNotification('New announcement: ' + newMsg.value.substring(0, 50) + '...', 'announcement', p.username));
  ...
  function downloadTranscriptPDF(student) {
    alert('📄 In a real implementation, this would generate a PDF using jsPDF.\n\nStudent: ' + student.name + '\nReference: TR-' + student.id + '-' + Date.now());
  }
  ...
  function viewSubmissions(assignmentId) {
    ...
    alert(html); // In real app, show in modal
  }
  ...
  function processPayment() {
    ...
    addNotification(`Payment of ₱${amount.toLocaleString()} received. Ref: ${refNum}`, 'payment', currentUser);
    ...
    container.innerHTML = filtered.map(b => `
      <div class="book-card">
        ...
        <div class="book-info">
          ...
          <p><strong>Available:</strong> ${b.available}/${b.copies}</p>
          <span class="book-status ${b.available > 0 ? 'available' : 'borrowed'}">
            ${b.available > 0 ? '✅ Available' : '❌ All Borrowed'}
          </span>
          ${role === 'student' && b.available > 0 ? 
            `<button onclick="borrowBook('${b.id}')" style="width: auto; background: var(--main-blue); color: white; margin-top: 10px;">📚 Borrow</button>` : ''
          }
        </div>
      </div>
    `).join('');
  ...
  function borrowBook(bookId) {
    ...
    if(book.available > 0) {
      ...
      saveData();
      addNotification(`You borrowed "${book.title}". Due: ${new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString()}`, 'library', currentUser);
      alert('✅ Book borrowed successfully! Due in 7 days.');
      renderBooks();
    }
  }
  ...
  function registerAlumni() {
    const name = document.getElementById('alumniStudent').value;
    const contact = document.getElementById('alumniContact').value;
    const email = document.getElementById('alumniEmail').value;
    const college = document.getElementById('alumniCollege').value;
  ...
  function getCurrentUser() {
    ...
    if(role === 'student') return students.find(s => s.name === currentUser);
    if(role === 'teacher') return teachers.find(t => t.name === currentUser);
    if(role === 'parent') return parents.find(p => p.child === currentUser);
    return null;
  }
  ...
  function submitEnrollment() {
    alert('✅ Enrollment application submitted!\n\nReference Number: ENR-' + Date.now() + '\n\nPlease wait for admin approval.');
